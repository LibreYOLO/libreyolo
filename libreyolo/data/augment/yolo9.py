"""YOLO9 augmentation recipe: mosaic + HSV + flips, normalized-xyxy targets.

Key difference from the YOLOX recipe: outputs ``[class, x1, y1, x2, y2]`` in
normalized (0-1) coordinates on an RGB [0, 1] image, supports per-instance
segments (rasterized to downsampled masks) and OBB angle targets. Also used
by the RT-DETR and RT-DETRv2 trainers.
"""
...
from .geometry import letterbox_rgb01
...
class YOLO9TrainTransform:
    ...
    def __call__(self, image, targets, input_dim, segments=None):
        ...
        segments_o = copy_segments(segments_t)
        ...
        if random.random() < self.flip_prob:
            ...
            segments_t = flip_segments_lr(segments_t, width)
        else:
            image_t = image
        ...
        if random.random() < self.vertical_flip_prob:
            ...
            segments_t = flip_segments_ud(segments_t, height)
        ...
        segments_t = filter_segments(segments_t, mask)

        # Fallback to original if all boxes filtered
        if len(boxes_t) == 0:
            image_t, r = letterbox_rgb01(image_o, input_dim)
            boxes_t = boxes_o * r
            labels_t = labels_o
            angles_t = angles_o
            segments_t = transform_segments(
        ...
        if return_masks:
            # Polygons are authoritative here (prefer_dense=False): the
            # letterbox/mosaic placement above is tracked on polygon coords
            # and transform_segments drops dense masks by design.
...
class YOLO9ValTransform:
    ...
    def __call__(self, img, res, input_size):
        ...
        """
        img, _ = letterbox_rgb01(img, input_size, self.swap)
    ...
    def _get_mosaic_item(self, idx):
        ...
        for i, index in enumerate(indices):
            ...
            if len(labels) > 0:
                ...
                if has_segments:
                    tile_segments = transform_segments(
                        segments,
                        scale=scale,
                        padw=padw,
                        padh=padh,
        ...
        if has_segments:
            mosaic_segments = transform_segments(
        ...
        if len(mosaic_labels) > 0:
            ...
            if has_segments:
                mosaic_segments = filter_segments(mosaic_segments, mask)
