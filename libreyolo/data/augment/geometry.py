"""Geometric augmentations: affine, flip, letterbox."""
...
def mirror(image, boxes, prob=0.5):
    """Random horizontal flip of image + xyxy boxes."""
...
def letterbox(img, input_size, swap=(2, 0, 1)):
    """Letterbox resize + pad (114) + HWC->CHW transpose.

    Returns a float32 CHW image still in BGR 0-255 (YOLOX convention) and the
    resize ratio.
    """
    if len(img.shape) == 3:
...
# Back-compat alias: this function has historically been exported as ``preproc``.
preproc = letterbox


def letterbox_rgb01(img, input_size, swap=(2, 0, 1)):
    """Letterbox resize + pad (114) + BGR->RGB + CHW transpose + /255.

    Shared by the YOLO9 and YOLO-NAS train pipelines, whose pretrained weights
    expect RGB inputs scaled to [0, 1].
    """
    if len(img.shape) == 3:
        padded_img = np.ones((input_size[0], input_size[1], 3), dtype=np.uint8) * 114
    else:
