"""DETR-family augmentation recipe on torchvision.transforms.v2.

Shared by DEIM, D-FINE, DEIMv2, RT-DETRv4, and EC detection. The pipeline
mirrors the upstream DETR training recipes:
...
- Multi-scale ``DETRMultiScaleCollate`` collate that randomly resizes each
  batch over a window around the base size.
...
family trainers' ``on_forward`` does the pixel-to-normalized translation.
"""
...
def generate_scales(base_size: int, base_size_repeat: int) -> List[int]:
    """Mirror of the upstream DETR dataloaders' ``generate_scales``.
...
class DETRTrainTransform:
    ...
    Args:
        ...
        imagenet_norm: Apply ImageNet (mean, std) after /255. Needed by
            ViT-backed families (EC, DEIMv2 DINO sizes); HGNetv2-backed
            families were pretrained on /255-only inputs and leave it off.
        sanitize_min_size: ``SanitizeBoundingBoxes`` minimum box size.
    """
    ...
    def __call__(self, image: np.ndarray, targets: np.ndarray, input_dim):
        """Args follow LibreYOLO's per-sample transform contract.
...
class DETRPassThroughDataset:
    ...
    # Recipe subclasses point this at their transform class so the default
    # preproc and the isinstance checks below stay family-correct.
