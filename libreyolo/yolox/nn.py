...
class YOLOXHead(nn.Module):
    ...
    def forward(self, xin):
        """
        Forward pass - returns raw head outputs.

        Args:
            xin: List of feature maps from FPN

        Returns:
            List of outputs, each (B, 5+num_classes, H, W)
        ...
        for k, (cls_conv, reg_conv, x) in enumerate(
            ...
            # Concatenate: [reg(4), obj(1), cls(num_classes)]
...
class YOLOXModel(nn.Module):
    ...
    def forward(self, x):
        """
        Forward pass - returns head outputs.

        Args:
            x: Input tensor of shape (B, 3, H, W)

        Returns:
            List of outputs from each scale, each (B, 5+num_classes, H, W)
        """
    ...
    def get_loss_fn(self):
        ...
        Returns:
            YOLOXLoss instance configured for this model.
        """
