...
class DFL(nn.Module):
    ...
    def __init__(self, c1=16):
        super().__init__()
        self.c1 = c1
        # register_buffer ensures 'project' moves to the GPU automatically
        # and saves it in the state_dict, but does not require gradients
...
class DDetect(nn.Module):
    ...
    def _dist2bbox(self, distance, anchor_points, xywh=True, dim=1):
        ...
        if xywh:
            c_xy = (x1y1 + x2y2) * 0.5 # Changed to multiplication by 0.5 instead of div by 2
            wh = x2y2 - x1y1
