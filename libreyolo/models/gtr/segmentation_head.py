...
class SegmentationHead(nn.Module):
    ...
    def forward(
        ...
        # print(self.image_size, target_size)
        ...
        elif len(query_features) == 1:
            # Eval / single-feature path: run the full block stack so the spatial
            # features see the same depth as training's final mask, matching
            # forward_export. Avoids the train/eval mismatch where zip() would
            # only run block[0].
