"""Shared process boundary for optional external model runtimes.

Images and tensors cross private pipes as JSON plus typed array bytes, never
pickle. Each family supplies a worker script and a display name. A worker keeps
its loaded model until close(), timeout, interrupt, or owner collection.
"""
...
def _read_responses(stream, replies, runtime_name):
    try:
        for line in stream:
            if line.startswith(_PREFIX):
                replies.put(json.loads(line[len(_PREFIX) :]))
            elif line.strip():
                logger.info("%s: %s", runtime_name, line.rstrip())
    except (OSError, ValueError) as exc:
        replies.put({"transport_error": str(exc)})
    finally:
        replies.put({"transport_error": f"The {runtime_name} worker exited."})
        stream.close()


def _read_logs(stream, logs, runtime_name):
    try:
        for line in stream:
    ...
    def __init__(
        ...
        *,
        ...
        threading.Thread(
            ...
            args=(self._process.stdout, self._replies, self._runtime_name),
            daemon=True,
        ).start()
        self._log_thread = threading.Thread(
            target=_read_logs,
            args=(self._process.stderr, self._logs, self._runtime_name),
            daemon=True,
        )
    ...
    def _rpc(self, request):
        with self._lock:
            if not self._finalizer.alive or self._process.poll() is not None:
                raise RuntimeError(f"The {self._runtime_name} worker is closed.")
            self._request_id += 1
            ...
            try:
                ...
                self.close()
                raise RuntimeError(
                    f"{self._runtime_name} inference worker timed out."
                ) from exc
            except (BrokenPipeError, OSError) as exc:
                self.close()
                raise RuntimeError(f"Lost the {self._runtime_name} inference worker.") from exc
            ...
            if reply.get("id") != self._request_id:
                self.close()
                raise RuntimeError(
                    f"{self._runtime_name} worker response does not match the request."
                )
            if "error" in reply:
                error = RuntimeError(
                    f"{self._runtime_name} runtime: {reply['error']['message']}"
                )
