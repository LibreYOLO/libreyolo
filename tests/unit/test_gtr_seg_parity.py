"""Pinned upstream GTRSeg graph alignment with all four real seg checkpoints.

CPU substitutes the portable attention and normalization operators in the
reference, as in test_gtr_parity.py. Set GTR_UPSTREAM and GTR_SEG_CHECKPOINTS
(a directory holding gtrseg_{s,m,l,x}_coco.pth). No downloads.
"""
...
def test_upstream_seg_graph_alignment_with_portable_operators(monkeypatch):
    if not os.environ.get("GTR_UPSTREAM") or not os.environ.get("GTR_SEG_CHECKPOINTS"):
        pytest.skip("Set GTR_UPSTREAM and GTR_SEG_CHECKPOINTS")
    ...
    from gtr_reference.gtr.gtr import GTRSeg
    ...
    for size in ("s", "m", "l", "x"):
        ...
        sd = torch.load(
            pathlib.Path(os.environ["GTR_SEG_CHECKPOINTS"]) / f"gtrseg_{size}_coco.pth",
            weights_only=True,
            map_location="cpu",
        )["ema"]["module"]
        ours.load_state_dict(sd, strict=True)
        cfg = config(root.parent / f"configs/seg/coco_seg_finetune/gtrseg_{size}.yml")
        assert cfg["GTRTransformer"]["mask_downsample_ratio"] == 4
        ...
        with torch.no_grad():
            a = ours(x)
            b = upstream(x)
        assert set(a) == {"pred_logits", "pred_boxes", "pred_masks"}
        for k in a:
