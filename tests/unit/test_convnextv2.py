...
from libreyolo.models.convnextv2.nn import ARCH_DEFS, ConvNeXtV2, GRN
...
def test_size_recognition_and_filename(size):
    ...
    assert LibreConvNeXtV2.detect_size_from_filename(f"LibreConvNeXtV2{size}.pt") is None
...
def test_grn_nonzero_parameters_and_gradient():
    ...
    expected = grn.gamma * (x * (norm / (norm.mean(-1, keepdim=True) + 1e-6))) + grn.beta + x
...
def test_prediction_save_reload_and_rebuild(tmp_path):
    ...
    image = Image.fromarray(np.random.default_rng(3).integers(0, 256, (90, 140, 3), dtype=np.uint8))
    ...
    torch.testing.assert_close(restored.predict(image, imgsz=64)[0].probs.data, before, rtol=0, atol=0)
...
def test_raw_autoconversion(tmp_path):
    ...
    assert (model.family, model.size, model.task, model.nb_classes) == ("convnextv2", "atto", "classify", 7)
...
def test_preprocessing_matches_official_224():
    ...
    image = Image.fromarray(np.random.default_rng(5).integers(0, 256, (259, 413, 3), dtype=np.uint8))
